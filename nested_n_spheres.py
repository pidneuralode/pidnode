...
data_concentric = ConcentricSphere(data_dim, inner_range=(0., 1.5), outer_range=(3.2, 3.9),
                                   num_points_inner=400, num_points_outer=800)
...
for i, name in enumerate(names):
    print("Model:", name)
    histories_file_path = f"{output_directory}/{args.extra_name}{name}_histories.pkl"
    if os.path.exists(histories_file_path) and not args.visualize_features:
        ...
        for run_idx in range(num_runs):
            ...
            if args.visualize_features:
                ...
                multi_feature_plt(feature_history[::step], targets, f"{output_directory}/viz_features/{args.extra_name}{name}.pdf",
                                  name=alt_name)
                break
            else:
...
if args.visualize_results == 1 and not args.visualize_features:
    ...
    plt.savefig(f"{output_directory}/viz_train/point_cloud.jpg", transparent=True, bbox_inches='tight', pad_inches=0, dpi=500)
