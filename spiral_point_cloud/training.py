# Credit to https://github.com/EmilienDupont/augmented-neural-odes/blob/master/anode/training.py
import json
...
class Trainer():
    ...
    """
    def __init__(self, model, optimizer, device, loss,
    ...
    def _get_and_reset_nfes(self):
        ...
        if hasattr(self.model, 'odeblock'):  # If we are using ODENet
            try:
                iteration_nfes = self.model.odeblock.odefunc.df.nfe
                # Set nfe count to 0 before backward pass, so we can
                # also measure backwards nfes
                self.model.odeblock.odefunc.df.nfe = 0
            except AttributeError:
                iteration_nfes = self.model.odeblock.odefunc[-1].nfe
                self.model.odeblock.odefunc[-1].nfe = 0
        else:  # If we are using ODEBlock
            try:
                iteration_nfes = self.model.odefunc.df.nfe
                self.model.odefunc.df.nfe = 0
            except AttributeError:
                iteration_nfes = self.model.odefunc[-1].nfe
                self.model.odefunc[-1].nfe = 0
