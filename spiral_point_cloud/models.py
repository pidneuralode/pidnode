...
class pidnode_initial_velocity(nn.Module):
    ...
    def forward(self, x0):
        ...
        out = self.tanh(out)
        out = self.fc3(out)
        batch_size, neural_len = x0.shape
        v0 = torch.zeros(batch_size, neural_len).to(self.gpu)
...
class ODEBlock(nn.Module):
    ...
    def __init__(self, odefunc, t0_, tN_, tol, half=False, one_third=False, nesterov_algebraic=False, actv_k=None,
                 use_momentum=False, actv_output=None):
    ...
    def forward(self, x):
        ...
        if self.one_third:
            one_three = int(len(x)/3)
            h = out[:one_three]
            # if you decide to use the momentum
            if self.use_momentum:
                m = out[one_three:one_three*2]
                v = out[one_three*2:]
                out = torch.cat((h, m, v), dim=1)
            else:
                out = h
        return out
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_dim, out_dim):
        ...
        self.fc2 = nn.Linear(3*out_dim, out_dim)
