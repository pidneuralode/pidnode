# Credit to https://github.com/EmilienDupont/augmented-neural-odes/blob/master/viz/plots.py
...
def multi_feature_plt(features, targets, save_fig='', name=None):
    ...
    """
    alpha = 0.5
    color_select = ['red', 'green', 'blue', 'yellow']
    color = [color_select[torch.argmax(targets[i])] for i in range(len(targets))]
    ...
    if num_dims == 2:
        ...
        for i in range(len(features)):
            axarr[i].scatter(features[i][:, 0].numpy(), features[i][:, 1].numpy(),
                             c=color, alpha=alpha, linewidths=0, s=100)
    ...
    if name:
        fig.text(0.1, 0.5, name, va='center', rotation='vertical', fontsize=30)
...
def trajectory_plt(model, inputs, targets, timesteps, highlight_inputs=False,
    ...
    trajectories = model.odefunc.trajectory(inputs, timesteps).detach()
    # Features are trajectories at the final time
    features = trajectories[-1]

    if model.augment_dim > 0:
        aug = torch.zeros(inputs.shape[0], model.odefunc.augment_dim)
