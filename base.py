...
class NODEintegrate(nn.Module):
    ...
    def __init__(self, df, shape=None, tol=1e-5, adjoint=True, evaluation_times=None, recf=None,
                 nesterov_algebraic=None, activation_h=None, activation_output=None, time_requires_grad=True,
                 verbose=True):
        """
    ...
    def calc_algebraic_factor(self, z):
        ...
        dh = self.activation_output(
            bmul(k, m - bmul(3 / 2 * torch.pow(T, 1 / 2) * torch.exp(-T / 2) - 1 / 2 * 1 / k, h)))
...
class HeavyBallNODE(NODE):
    ...
    def forward(self, t, x):
        ...
        #         m = (3/2 * torch.pow(t, 1/2) * torch.exp(-t/2) - 1/2 * k_reciprocal) * h + k_reciprocal * dh
...
class NesterovNODE(NODE):
    def __init__(self, df, actv_h=None, corr=-100, corrf=True, use_h=False, full_details=False, nesterov_algebraic=True,
...
class High_NesterovNODE(NODE):
    """
    通过引入不可最小化步长的方式来进行了nesterov算法的高分辨率化操作
    """

    def __init__(self, df, actv_h=None, corr=-100, corrf=True, use_h=False, full_details=False, nesterov_algebraic=True,
                 actv_m=None, actv_dm=None, actv_df=None, sign=1, u=0.1, s=0.1):
        super().__init__(df)
        self.corr = Parameter([corr], frozen=corrf)
        self.sp = nn.Softplus()
        self.sign = sign  # Sign of df
    ...
    def forward(self, t, z):
        """
        $$ h' = m-sqrt(s)*df(t, h) $$
        $$ m' = -2sqrt(u)m+(sqrt(us)-1)*df(t, h) $$
        Parameters
        ----------
        t 动态系统或者回归问题中的真实时间或者固定的步长0-1
        z [h, m]表示为二阶系统的扩展项操作

        Returns
        -------
        经过ode_solve之后的前向计算出输出信息
        """
        self.nfe += 1
        h, m = torch.split(z, 1, dim=1)
        # 将新组装的变量也推广到新系统
        dh = m - math.sqrt(self.s) * self.df(t, h)
        dm = -2 * math.sqrt(self.u) * m + (math.sqrt(self.u * self.s) - 1) * self.df(t, h)
        out = torch.cat((dh, dm), dim=1)
        if self.elem_t is None:
            return out
        else:
...
class Generalized_High_NesterovNODE(NODE):
    """
    通过引入不可最小化步长的方式来进行了nesterov算法的高分辨率化操作，同时通过限制隐藏层和隐藏层微分量的操作来减少计算过程中梯度消失的问题
    """
    ...
    def forward(self, t, z):
        ...
        h, m = torch.split(z, 1, dim=1)
        # 将新组装的变量也推广到新系统
        dh = self.actv_df(m - math.sqrt(self.s) * self.df(t, h)) * self.sign
        dm = self.actv_dm(-2 * math.sqrt(self.u) * m + (math.sqrt(self.u * self.s) - 1) * self.df(t, h)
                          + self.sp(self.corr()) * h)
        out = torch.cat((dh, dm), dim=1)
        if self.elem_t is None:
            return out
...
class HighNesterovNODE2(NODE):
    """
    引入梯度关于时间的偏导作为减少震荡的操作
    """

    def __init__(self, df, actv_h=None, corr=-100, corrf=False, gamma_guess=-3.0, gamma_act='sigmoid',
                 siga=-0.5, use_h=False, full_details=False, nesterov_algebraic=True,
                 actv_m=None, actv_dm=None, actv_df=None, sign=1):
        super().__init__(df)
        self.gamma = Parameter(torch.Tensor([gamma_guess]), frozen=False)
        self.gammaact = nn.Sigmoid() if gamma_act == 'sigmoid' else gamma_act
        self.corr = Parameter(torch.Tensor([corr]), frozen=corrf)
        self.siga = Parameter(torch.Tensor([siga]), frozen=True)
        self.sp = nn.Softplus()
        self.sign = sign  # Sign of df
        self.actv_h = nn.Identity() if actv_h is None else actv_h  # Activation for dh, GNNODE only
        self.actv_m = nn.Identity() if actv_m is None else actv_m  # Activation for dh, GNNODE only
    ...
    def forward(self, t, x, multiforecast=None, retain_grad=False):
        ...
        if self.rnn_out:
            for i in range(n_t):
                self.ode.update(t[i])
                h_ode[i] = \
                    odeint(self.ode, h_rnn[i], self.evaluation_times, atol=self.tol, rtol=self.tol, method=self.method)[
                        -1]
                if self.nesterov_algebraic:
