...
def train(model, optimizer, trdat, tsdat, args, modelname, testnumber=0, evalfreq=1, lrscheduler=False,
    ...
    while epoch < args.niters:
        ...
        try:
            gamma = model[1].df.gamma.val.detach().cpu().numpy()[0]
        except Exception:
            gamma = 0
        try:
            corr = model[1].df.corr.val.detach().cpu().numpy()[0]
        except Exception:
            corr = 0
        ...
        if time_arr[epoch - 1] > args.timeout or \
            forward_nfe_arr[epoch - 1] > args.nfe_timeout or backwardnfe > args.nfe_timeout:
            break
        if epoch % evalfreq == 0:
            model[1].df.nfe = 0
            test_time = time.time()
