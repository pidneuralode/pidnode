...
class MODEL(nn.Module):
    def __init__(self, res=False, cont=False):
        ...
        nhid = 24
        self.cell = NesterovNODE(tempf(nhid, nhid), use_h=True)
        self.rnn = temprnn(17, nhid, nhid, res=res, cont=cont)
        evaluation_times = torch.Tensor([1, 2])
        self.ode_rnn = ODE_RNN_with_Grad_Listener(self.cell, self.rnn, (2, nhid), None, tol=1e-7, nesterov_algebraic=True, time_requires_grad=False, evaluation_times=evaluation_times)
